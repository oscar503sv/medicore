  ...
  return (
    <header className="flex h-14 shrink-0 items-center justify-end gap-1 border-b border-line bg-surface px-4">
      ...
      <div className="relative ml-1">
        <button onClick={() => setMenuOpen((v) => !v)} className="flex items-center gap-2 rounded-lg p-1 hover:bg-surface-2">
          ...
          <>
            ...
            <div className="absolute right-0 top-11 z-20 w-52 rounded-lg border border-line bg-surface p-1 shadow-lg animate-pop-in">
              <div className="border-b border-line-soft px-3 py-2">
                <p className="text-sm font-medium text-tx">{session?.name}</p>
                <p className="text-xs text-tx-3">{t(`role.${session?.role}`)}</p>
              </div>
              <button
                onClick={() => {
                  logout()
